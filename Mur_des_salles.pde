...
bouton[0].bouton();
 ...
 if(bouton[0].appuyer==false) { timefinol=0;} else { if(timefinol==0) {timefinol=1; laserson.close(); laserson = minim.loadSnippet("sonlaser.mp3");  laserson.play();}}
...
//laser a désactivé :
if(levier[1].timelevier==119) {laserson.close(); laserson = minim.loadSnippet("sonlaser.mp3");  laserson.play();}
...
for(int i=0; i<s1vide.length; i++) { s1vide[i].meurtv();}
 ...
 if(fin==false) {image(a,430,350,120,120);}
 if(bouton[5].sonbouton==1) {laserson.close(); laserson = minim.loadSnippet("sonlaser.mp3");  laserson.play();}
 if(bouton[5].appuyer==false) {ascenseuro=false;  fill(255,0,0,100);  rect(460,390,60,70); fill(255,255);}
