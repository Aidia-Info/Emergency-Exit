...
//mort dans le vide
 ...
 if(meurtvide==20) {    non.close(); non= minim.loadSnippet("non.mp3"); non.play();   }
...
//mort dans un laser
 if(meurtlaser>0 && meurtlaser<100) { if(meurtlaser==1) {active=false; taser.close(); taser= minim.loadSnippet("taser.mp3"); taser.play();    mortelec.index=0;} meurtlaser++; mortelec.anim(x-60,y-60,120,120);}
...
if(salle==2 && x>=width/2-25 && x <=width/2+40 && y>=height/2-15 && y<=height/2+50 && fin==false) {    boutone.anim(x+5,y-70,50,50);       if(touche['e']==true) {fin=true;}
} 
