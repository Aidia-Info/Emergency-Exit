...
AudioSnippet portefin;
AudioSnippet lootson;
AudioSnippet cubeson1;
AudioSnippet cubeson2;
AudioSnippet tuyauson;
AudioSnippet boutonson1;
AudioSnippet boutonson2;
AudioSnippet laserson;
AudioSnippet tirson;
AudioSnippet switch1;
AudioSnippet switch2;
AudioSnippet ding;
AudioSnippet tpson;
AudioSnippet finfin;
AudioSnippet non;
...
void setup() {
  ...
  portefin = minim.loadSnippet("portefin.mp3");
  lootson = minim.loadSnippet("sonloot.mp3");
  cubeson1 = minim.loadSnippet("cubeprit.mp3");
  cubeson2 = minim.loadSnippet("cubepose.mp3");
  boutonson1 = minim.loadSnippet("sonboutongrave.mp3");
  boutonson2 = minim.loadSnippet("sonboutonaigu.mp3");
  laserson = minim.loadSnippet("sonlaser.mp3");
  tirson = minim.loadSnippet("sontire.mp3");
  switch1 = minim.loadSnippet("switcharmeaigu.mp3"); 
  switch2 = minim.loadSnippet("switcharmegrave.mp3");
  ding = minim.loadSnippet("ding.wav");
  tpson = minim.loadSnippet("tpson.mp3");
  finfin = minim.loadSnippet("fin.mp3");
  non= minim.loadSnippet("non.mp3");
  taser= minim.loadSnippet("taser.mp3");
  //
...
//Image menu
 ...
 //Image cube
 cubeanim= loadImage("cubeanim.png");

 //Image Aidia
 aidia= loadImage("aidia.png");
...
//Message / x / y / image --> false : mystère ; true : perso / Commencement à 0 : false / à 1000 : true / vitesse 1 de base 
 ...
 msg11=new Message("Bien, le sujet réagit normalement, on peut continuer.",100,80,false,false,false,1.1);
 msg12=new Message("Cette arme vous permet de vous téléporter.",100,80,false,true,true,1.1);
 msg13=new Message("Visez, clic gauche, recliquez et voilà.",100,80,false,true,true,1.1);
 msg14=new Message("Passez la barrière électrique. Mais attention !",100,80,false,true,true,1.1); 
 ...
 msg21=new Message("Utilise la molette pour changer d'arme.",100,80,false,false,false,1.1);
 msg22=new Message("Em main, tu pourras activer certaines choses.",100,80,false,true,true,1.1);
 
 msg31=new Message("Voici une amélioration : clic droit.",100,80,false,false,false,1.1);
 msg32=new Message("Tu ne pourras plus te téléporter...",100,80,false,true,true,1.1);
 msg33=new Message("Mais tu pourras téléporter le cube ! De plus,",100,80,false,true,true,1.1);
 msg34=new Message("la boule pourra traverser les murs de sa couleur.",100,80,false,true,true,1.1);
 msg35=new Message("N'hésite pas à changer lorsqu'elle est lancée.",100,80,false,true,true,1.1);
 msgp6= new Message("Ça commence à devenir compliqué...",600,280,true,true,false,1.1);
 
 msg41=new Message("Dans cette salle il y aura des tuyaux.",100,80,false,false,false,1.1);
 msg42=new Message("La boule entre par un côté, et ressort de l'autre.",100,80,false,true,true,1.1);
 msg43=new Message("Vise le point noir au milieu et tire.",100,80,false,true,true,1.1);
 msg44=new Message("Attention néanmoins aux barrières rouges...",100,80,false,true,true,1.1);
 msg45=new Message("La boule n'apprécie guère ce genre de choses.",100,80,false,true,true,1.1);
...
if(timecredit/2<=255) {  
 ...
 if(molette==-1 || molette==1) {arme=-arme; molette=0;  if(arme==-1) { switch1.close(); switch1 = minim.loadSnippet("switcharmegrave.mp3");  switch1.play();  } else { switch2.close(); switch2= minim.loadSnippet("switcharmeaigu.mp3");  switch2.play();      }    }
...
// stroke(255,0,0); strokeWeight(3); line(x ,y ,mouseX,mouseY); noStroke();
 ...
 if(Vie==false){
  ...
  sallecube=0; xc=900; yc=40;
  timemort--;
 }
 
  if((touche['1']==true && touche['2']==true && touche['3']==true) || timemort==0) {x=100; y=700; Vie = true; meurtvide=0; meurtlaser=0; active=true; dureT=350; salle=0; timevers=0; cubepris=false; timemort=200;}
fill(255,255,255);
textSize(20);
...
if(Vie==true && menupause==false && fin2==false) {timer();}
if(fin2==true) {textSize(70); fill(100,0,0);  text("Temps : "+minute+"."+seconde+"."+mili*10/6,300,200);        text("Nombre de morts :"+morts,250,300);}
...
if(daba==true) {succesd.anims();}
 ...
 if(timearme1==0 && salle==0) {guidedebut();}
  ...
  if(timearme1==240) {ding.close(); ding = minim.loadSnippet("ding.wav");  ding.play();} 
  if(timearme1==300) { guide2(); }
  
  if(timearme2==440) {ding.close(); ding = minim.loadSnippet("ding.wav");  ding.play();}
  if(timearme2==500) { guide3(); }
  
  if(timearme3==440) {ding.close(); ding = minim.loadSnippet("ding.wav");  ding.play();}
  if(timearme3==500) { guide4(); }
  
  if(declancheur==40) {ding.close(); ding = minim.loadSnippet("ding.wav");  ding.play();}
...
{depX=-depX;
plop.close(); plop = minim.loadSnippet("plop.mp3");
plop.play();
}
    //HITBOX EN Y
...
void on(boolean lock){
 ...
 boutone2.anim(xp-50,yp-20,50,50);
 if(touche['e']==true&& lock==true && arme==-1) {timep=1; door.play(); door.rewind();}}
 
 if(((x>xp+22+70 && x<xp+32+70)||(x>xp-22+70 && x<xp-12+70) && y>yp+10 && y<yp+80) && timep==93 && touche['e']==true&& lock==true) {timep=95;  doori.play(); doori.rewind();}
 }
...
if(timelevier>0 && timelevier<120) {timelevier++; activel.anim(xl,yl,120,120);}
 if(timelevier==119) {active=true; x=xl+60; y=yl+75; activer=true;}
  if(timelevier==120)  {leviera.anim(xl,yl,120,120); if(x>xl+20 && x<xl+100 && y>yl+50 && y<yl+100) {boutone.anim(xl+35,yl,55,55); if(touche['e']==true) {timelevier=131; active=false;} } }
   ...
   if(timelevier==50||timelevier==180) {leviers.close(); leviers = minim.loadSnippet("levier.mp3");  leviers.play();
 }
...
void bouton() {
  ...
  if((xc>xbouton && xc<xbouton+50 && yc>ybouton && yc<ybouton+50 && cubepris==false)||(x>xbouton && x<xbouton+50 && y>-10+ybouton && y<ybouton+30)) {if(sonbouton==4) {sonbouton=0;}    appuyer=true; if(sonbouton<2) { sonbouton++; }}  else{appuyer=false;  if(sonbouton<4) { sonbouton++; }} 
  
  if(sonbouton==1) {boutonson1.close(); boutonson1 = minim.loadSnippet("sonboutongrave.mp3");  boutonson1.play();}
  if(sonbouton==3) {boutonson2.close(); boutonson2 = minim.loadSnippet("sonboutonaigu.mp3");  boutonson2.play();}
}
}
